...
st.write(f"### 現在の難易度: **{st.session_state.difficulty[0]}**")
st.info(st.session_state.message)

# --- 入力フォーム ---
if not st.session_state.game_over and st.session_state.win_count < 5:
    with st.form(key='guess_form', clear_on_submit=True):
        guess = st.number_input("1-100の数字を入力してください", min_value=1, max_value=100, key="input")
    ...
    if submit:
        ...
        if diff == 0:
            ...
            st.session_state.message = f"はずれ！ 減少ポイント: -{penalty}。 {hint}"
